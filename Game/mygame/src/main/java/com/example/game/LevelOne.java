...
public class LevelOne{
  ...
  public void drawMap(String filePath, int gameBarrierCol, int gameBarrierRow){
    try{
      InputStream is = getClass().getResourceAsStream(filePath);
      BufferedReader br = new BufferedReader(new InputStreamReader(is));
      int col = 0;
      int row = 0;
      while(col< gameBarrierCol && row< gameBarrierRow){
        String line = br.readLine();
        while(col<gameBarrierCol){
          String numbers[] = line.split(" ");
          int num = Integer.parseInt(numbers[col]);
          mapCells[col][row] = num;
          col++;
        }
        if(col == gameBarrierCol){
          col = 0;
          row++;
        }
      }
  ...
  public void draw(Graphics2D g2d, int cellSize, int gameBarrierCol, int gameBarrierRow) {
    int col = 0;
    int row = 0;
    int x = 0;
    int y = 0;
